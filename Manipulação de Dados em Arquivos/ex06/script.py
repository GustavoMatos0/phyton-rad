...
def processar_arquivo(arquivo_origem, arquivo_destino):
    try:
        ...
        print(f"Arquivo {arquivo_origem} não encontrado. {e}")
        return
    except PermissionError as e:
        print(f"Sem permissão para ler {arquivo_origem}. {e}")
        return
    ...
    try:
        ...
        print(f"Sem permissão para escrever em {arquivo_destino}. {e}")
    except Exception as e:
        print(f"Erro inesperado ao escrever em {arquivo_destino}. {e}")
        

def main():
    dir_trabalho = "dir_trabalho_ex06"
    arquivo_origem = os.path.join(dir_trabalho,"arquivo_origem.txt")
    arquivo_destino = os.path.join(dir_trabalho,"arquivo_destino.txt")
